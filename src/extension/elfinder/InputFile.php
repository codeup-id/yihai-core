...
class InputFile extends InputWidget{
	...
	public function init()
	{
		...
		if(empty($this->language))
			$this->language = ElFinder::getSupportedLanguage(Yihai::$app->language);
	...
	public function run()
	{
		...
		if (!empty($this->multiple))
			$this->getView()->registerJs("codeup.elFinder.register(" . Json::encode($this->options['id']) . ", function(files, id){ var _f = []; for (var i in files) { _f.push(files[i].url); } \$('#' + id).val(_f.join(', ')).trigger('change', [files, id]); return true;}); $(document).on('click','#" . $this->buttonOptions['id'] . "', function(){codeup.elFinder.openManager(" . Json::encode($this->_managerOptions) . ");});");
		else
			$this->getView()->registerJs("codeup.elFinder.register(" . Json::encode($this->options['id']) . ", function(file, id){ \$('#' + id).val(file.url).trigger('change', [file, id]);; return true;}); $(document).on('click', '#" . $this->buttonOptions['id'] . "', function(){codeup.elFinder.openManager(" . Json::encode($this->_managerOptions) . ");});");
	}
}
