...
class ElFinder extends BaseWidjet{
	...
	public static function getManagerUrl($controller, $params = [])
	{
		...
		return Yihai::$app->urlManager->createUrl($params);
	}
	...
	public function init()
	{
		if(empty($this->language))
			$this->language = self::getSupportedLanguage(Yihai::$app->language);
	...
	public function run()
	{
		...
		if(!empty($this->callbackFunction)){
			...
			$this->getView()->registerJs("codeup.elFinder.register(".Json::encode($this->id).",".Json::encode($this->callbackFunction).");");
		}
	}
}
