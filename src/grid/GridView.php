...
class GridView extends \dosamigos\grid\GridView
{
    ...
    public function renderSummary()
    {
        ...
        if (!isset($size[$default_pageSize])) $size[$default_pageSize] = $default_pageSize;
        $active = $default_pageSize;
        $sessId = 'pager-' . $this->id;
        $parent = parent::renderSummary();
        return Html::tag('span',
                Html::beginForm('', 'get', ['data-pjax' => true, 'style' => 'width:250px']) .
                Html::dropDownList($sessId, $active, $size, ['onchange' => 'if($){$(this).closest(\'form\').submit();}else{this.form.submit()}', 'class' => 'folrm-control']) .
                Html::endForm()) .
            $parent;
